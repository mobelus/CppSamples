...
### STD::MOVE
Ответ: Она преобразует любой входной тип к RValue-ссылке!
...
**Зачем нужен std::forward** - Концептуально: Нужна для того, чтобы осуществлять "Проброс типа" того типа который нам нужен, через шаблонную функцию так чтобы на выходе этот тип либо в неизменном виде остался. Ибо в результате работы с сылками при работе склейки ссылок мы можем получить не тот тип, на выходе, который мы хотим. Для решения этой проблемы и есть std::forward.
