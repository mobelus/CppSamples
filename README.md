...
**HANDLE** - 32разрядный INT - Дескриптор какого-то объекта Ядра. У каждого процесса есть таблица Дескрипторов.

**ПРОЦЕСС** - 1. исполняемая программа 2. некое состояние выполнения операции
...
- СТЕК программы
- связанный набор регистров (1. счётчик команд, регистр с адресами следующей 


- Клиент использует QueryInterface-метод чтобы запросить интерфейс, содержащийся в СОМ-компоненте
```
СОМ-комп.1
-- A
-- B
...
A,B,...Y,Z - Интерфейсы
```
- у любого СОМ интерфейса/компонента/класса есть GUID
- СОМ компонент хочет воспользоваться интерфейсом, и вызывает AddRef()
- СОМ компоненту больше не нужен интерфейс - вызываем Release()
- СОМ компонент остаётся загруженный в память пока счётчик ссылок на него не станет равен нулю
- CLSID - используется в СОМ-библиотеках, чтобы загрузить СОМ-компонент
...
### Есть преимущества make_shared() перед shared_ptr<> ?
1. Внутри shared_ptr используется счётчик ссылок.
2. Рассмотрим 2 случая 1ЫЙ В конструктор передаём результат shared_ptr, и 2ОЙ в конструктор передаём вызов make_shared. В случае с make_shared произойдёт ОДНО выделение памяти, а в случае с shared_ptr будет выделенеие 2 раза.
